...
def group_by_block_and_month(df):
    ...
    grouped = df.groupby(['Block', 'month'], as_index=False)[sum_cols].sum()
...
def group_by_block(df):
    sum_cols = ['Ruda', 'fRuda', 'Cu', 'fCu', 'Ag', 'fAg']
    grouped = df.groupby(['Block'], as_index=False)[sum_cols].sum()
...
def assign_block_numbers(data, col_idx):
    ...
    Assigns consecutive block numbers from top to bottom:
      - Every time we see a row containing 'того', that row is a delimiter
        (i.e., not assigned any block).
      - The block number is incremented only if there is at least one
        non-'того' row before the current 'того' row.
      - All preceding non-'того' rows belong to the current block number.

    Does not call .copy() explicitly; uses .loc and .drop() in-place.
    """
    ...
    # 1) Assign the new 'Block' column
    data.loc[:, 'Block'] = block_assignments

    # 2) Convert only valid (non-None) blocks to integer
    keep_mask = data['Block'].notna()  # True = keep
    data.loc[keep_mask, 'Block'] = data.loc[keep_mask, 'Block'].astype(int)

    # 3) Drop rows with None blocks (the 'того' rows), in place
    drop_index = data.index[~keep_mask]
